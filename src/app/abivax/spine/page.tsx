...
function computeLatestDigest(notes: Note[]) {
  ...
  const truthsNow = dedupe(
    last5.flatMap((n) => n.summary?.truthsNow ?? [])
  ).slice(0, 5);
  const nextConstraints = dedupe(
    last5.flatMap((n) => n.summary?.nextConstraints ?? [])
  ).slice(0, 3);
  ...
  if (notes.length >= 2) {
    ...
    changed = dedupe(
      [...latestSet].filter((x) => !prevSet.has(x))
    ).slice(0, 3);
  }
...
export default async function SpinePage() {
  const [spineState, notesData] = await Promise.all([
    loadSpineState(),
    loadNotes(),
  ]);
    ...
    <div className="space-y-8">
      <header>
        ...
        <p className="mt-1 text-sm text-slate-500">
          North star + latest digest. Use the sidebar to drill into People,
      ...
      <div className="rounded-lg border border-amber-900/50 bg-amber-950/30 px-4 py-2.5 text-xs text-amber-200/90">
        Paste new notes into Cursor → updates /data/abivax/notes.json
      </div>
